...
class impeeIn extends InputPort
{
    ...
    // and sends it out over the UART5/7. We'll also toggle the rxLed
    function set(c)
    {
        foreach(i, val in c){
            hardware.uart57.write(val.tointeger());
            server.show("letter :" + val);
            server.log("index :" + i);
            server.log(format("letter: %d", val.tointeger() ));
        }
...
local impeeOutput = OutputPort("impOutput", "string");  // set impeeOutput as a string
...
function pollUart()
{
    ...
    {
        server.log(format("found: %c", byte)); // send the character out to the server log. Optional, great for debugging
        //  impeeOutput.set(byte);  // send the valid character out the impee's outputPort
        if (format("%c", byte) == "#") {
            impeeOutput.set(format("%s",bufstr));
            server.log("Sending to output");
            server.log(bufstr);
            bufstr = "";
            reading = false;
        }
        if (reading) {
            bufstr = format("%s%c", bufstr, byte);
        }
        if (format("%c", byte) == "$") {
            reading = true;
            bufstr = "";
...
imp.configure("Entrance", [impeeInput], [impeeOutput]);
